...
class Curva:
    ...
    def interpolacion(s,p:int=0,r:float=0.0) -> list:
        """ r es el parámetro spbre la curva [0,1)
        ...
        #============================
            ...
            zm2:np.float64 = 18.0+u12*am2*(-459.0+am2*(382.5+am2*(-156.5+am2*(41.5-2.5*am2))))
            xi.append(zp1*s.x[ip1]+z*s.x[i]+zp2*s.x[ip2]+zp3*s.x[ip3]+zm1*s.x[im1]+zm2*s.x[im2])
            for j in range(1,s.dim):
                xi.append(zp1*s.x[ip1+j*s.n]+z*s.x[i+j*s.n]+zp2*s.x[ip2+j*s.n]+zp3*s.x[ip3+j*s.n]+zm1*s.x[im1j*s.n]+zm2*s.x[im2j*s.n])
...
#=================================================================================
# Función xspline crea el z-spline de un conjunto de puntos en dim dimensiones,
